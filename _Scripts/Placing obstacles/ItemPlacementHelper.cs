...
public class ItemPlacementHelper
{
        ...
        HashSet<Vector2Int> roomFloorNoCorridor)
    {
        ...
        {
            int neighboursCount8Dir = graph.GetNeighbouts8Directions(position).Count;
            PlacementType type = neighboursCount8Dir < 8 ? PlacementType.NewrWall : PlacementType.OpenSpace;
            ...
            if (type == PlacementType.NewrWall && graph.GetNeighbouts4Directions(position).Count > 0)
                continue;
    ...
    public Vector2? GetItemPlacementPositin(PlacementType placementType, int iterationsMax, Vector2Int size, bool addOffset)
    {
        ...
        while (iteration < iterationsMax)
        {
            ...
            if (itemArea > 1)
            {
                ...
                tileByType[PlacementType.NewrWall].ExceptWith(placementPositions);
            }
            else
            {
